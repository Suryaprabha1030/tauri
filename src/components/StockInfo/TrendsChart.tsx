...
const TrendsChart = React.memo(({ data, initial, dropdown }: TrendsChartProps) => {
            ...
            <div className="border border-gray-200 p-2 w-[33%] max-sm:w-full">
                <div className="relative flex flex-col" onClick={handleDropDown}>
                    <div className="flex flex-row justify-between p-1 border border-gray-200 rounded-md hover:cursor-pointer">
                        ...
                        <Image  className={`${isDropdown ? 'rotate-180' : ""}`} src={"/svg/downChevron.svg"} alt="" width={16} height={16} />
                    </div>
                    {isDropdown &&
                        <div ref={dropdownRef} className="flex flex-col w-full shadow-md absolute mt-10 z-10 bg-white rounded-md border border-gray-200">
                            {Object.keys(values)?.map(section =>
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        setIsDropdown(false)
                                        dispatch(setSelectedChart({ index: initial, value: section }))
                                    }}
                                >
                                    {dropdown?.includes(section) ? `${section} (Chosen)` : `${section}`}
                                </button>)}
                        </div>
                    }
                </div>
