...
const PeersChart = React.memo(({ data }: any) => {
    ...
    const series = [
        { name: tooltipValues[currentChart], data: currentValue },
    ]
        ...
        <div className="w-full">
            <div className="relative flex items-center justify-between p-2">
                ...
                <button
                    ...
                    <Image className={`${isDropdown ? 'rotate-180' : ""} ml-auto`} src={"/svg/downChevron.svg"} alt="" width={16} height={16} />
                </button>
                {isDropdown &&
                    <div ref={dropdownRef} className="z-20 text-[0.85rem]  absolute top-12 right-4 border rounded-md bg-white border-gray-100 bg-green shadow-md">
                        {fields.map(item => {
                            return (
                                <div key={item} className={`${currentChart === item ? "bg-[#4CA858] text-white hover:bg-[#4CA858]" : "hover:bg-gray-100"} z-10 p-1 cursor-pointer`}
                                    onClick={() => {
