...
const MultiChartData = (data) => {
  ...
  const transformedStrikes = oiArr.map((item) => ({
    strike_price_option: item.strike_price_option,
    data: item.data
      .map((curr) => ({ [curr.created_at]: curr.oi }))
      .sort(
        (a, b) =>
          new Date(Object.keys(a)[0]).getTime() -
          new Date(Object.keys(b)[0]).getTime()
  ...
  function filterMatchingKeys(arr1, arr2) {
    const keysSet = new Set(arr1.map((obj) => Object.keys(obj)[0]));

    const filteredArr2 = arr2.map((item) => ({
      ...item,
      data: item.data.filter((dataObj) => keysSet.has(Object.keys(dataObj)[0])),
    }));

    const validKeysInArr2 = new Set(
      filteredArr2.flatMap((item) =>
        item.data.map((dataObj) => Object.keys(dataObj)[0])
      )
    );

    return {
      filteredArr1: arr1.filter((obj) =>
  ...
  const ltp1 = filteredArr1.map((obj) => Object.values(obj)[0]);

  const series1 = filteredArr2.map((item) => ({
  ...
  const xAxisValues = filteredArr1.map((obj) =>
  ...
  return {
    ...
    xAxisValues,
    ltp: ltp1,
    oiData: filteredArr2?.map((item) =>
      item.data.map((obj) => Object.values(obj)[0])
    ),
  };
};
