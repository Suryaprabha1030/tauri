...
const ImportStrategyModal = (props: {
  hideModal: () => void;
  importBluePrint: (blueprint: any, strategyName: string) => void;
}) => {
  ...
  const getReadyMade = async (name: string) => {
    ...
    await strategyApi
      ...
      .then((response) => {
        ...
        props.importBluePrint(
          response.data.result,
          response.data.strategy_name
        );
      });
  };
                ...
                <ul className="text-black-500 max-h-[calc(60vh-10rem)] overflow-y-scroll rounded-lg text-left text-sm">
                  {values.strategies.map((item: UserStrategy) => (
                    <li
                      key={item.id}
                      onClick={() =>
                        props.importBluePrint(item.data, item.name)
                      }
                      className="cursor-pointer bg-white px-6 py-2 text-center hover:bg-z-green-300"
                    >
                      {item.name}
                    </li>
                  ))}
